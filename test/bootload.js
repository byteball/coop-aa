...
chai.use((_chai, utils) => {
	...
	chai.Assertion.addMethod("deepCloseTo", function (expected, digits, msg) {
		let actual = utils.flag(this, "object");

		new chai.Assertion(typeof actual).to.be.eq(typeof expected, 'type mismatch')

		if (typeof actual !== 'object') {
			if (typeof actual === 'number' && digits !== undefined)
				return this.closeTo(expected, expected / 10 ** digits, msg);
			else
		...
		if (Array.isArray(actual)) {
			...
			for (let i = 0, imax = actual.length; i < imax; ++i) {
				new chai.Assertion(actual[i]).deepCloseTo(expected[i], digits, msg + "[" + i + "]");
			}
		}
		else {
			new chai.Assertion(Object.keys(actual).length).to.be.eq(Object.keys(expected).length, 'object length mismatch')
			for (let key in actual) {
				new chai.Assertion(actual[key]).deepCloseTo(expected[key], digits, msg + "[" + key + "]");
			}
		}
